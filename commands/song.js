...
function safeFileName(title) {
    return title.replace(/[^a-zA-Z0-9\s-_]/g, '').trim().slice(0, 80) || 'audio';
}
...
module.exports = {
    ...
    async execute({ sock, msg, args, jid }) {
        ...
        if (!query) {
            return sock.sendMessage(jid, {
                text:
                    ...
                    '✨ *Examples:*\n' +
                    '.song https://youtube.com/watch?v=60ItHLz5WEA\n' +
        ...
        const cleanFileName = `${safeFileName(title)}.mp3`;

        // ⭐ Envoi de l'audio avec le nom de fichier propre
        await sock.sendMessage(jid, {
            audio: buffer,
            mimetype: 'audio/mpeg',
            fileName: cleanFileName, // 👈 Permet la sauvegarde sous le bon nom
            ptt: false,
            contextInfo: {
                externalAdReply: {
                    title: title,
